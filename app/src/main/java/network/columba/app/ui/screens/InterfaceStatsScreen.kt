...
fun InterfaceStatsScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(state.interfaceEntity?.name ?: stringResource(R.string.interface_stats_interface_stats)) },
                navigationIcon = {
            ...
            if (needsUsbPermission && isEnabled && !isOnline) {
                ...
                Surface(
                    ...
                    Column(
                        ...
                        Spacer(modifier = Modifier.height(4.dp))
                        Text(
                            text = stringResource(R.string.interface_stats_grant_permission_to_connect_to_the),
...
private fun RNodeSettingsCard(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            spreadingFactor?.let {
                StatsInfoRow(label = stringResource(R.string.interface_stats_spreading_factor), value = "SF$it")
            }
            codingRate?.let {
                StatsInfoRow(label = stringResource(R.string.interface_stats_coding_rate), value = "4/$it")
            }
            txPower?.let {
                StatsInfoRow(label = stringResource(R.string.interface_stats_tx_power), value = "$it dBm")
            }
            interfaceMode?.let {
